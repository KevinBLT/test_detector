...
class _TextDetector {
  ...
  async _wordHeatMap(imageData) {
    let tensor     = this._detectionTensor(imageData),
        heatmap    = new OffscreenCanvas(imageData.width, imageData.height),
    ...
    await tf.browser.toPixels(prediction, heatmap);
  ...
  async _calculateBoundingBoxses(imageData) {
    let heatmap = await this._wordHeatMap(imageData), 
      ...
      heatmap.getContext('2d').getImageData(
        0, 0, heatmap.width, heatmap.height
      )
    );
    ...
    for (let i = 0, coords, w, h; i < boundingBoxes.length; i++) {
      ...
      boundingBoxes[i] = new TextDecodeResult([
        {x: w * coords[0].x, y: h * coords[0].y},
        {x: w * coords[1].x, y: h * coords[1].y},
        {x: w * coords[2].x, y: h * coords[2].y},
        {x: w * coords[3].x, y: h * coords[3].y},
      ]);
  ...
  async detect(imageData) {
    ...
    return new Promise(async (resolve) => {
      let boundingBoxes = await this._calculateBoundingBoxses(imageData),
          crops         = [],
          image         = new OffscreenCanvas(imageData.width, imageData.height),
      ...
      for (let bbox of boundingBoxes) {
        crops.push(context.getImageData(
          bbox.boundingBox.x,
          bbox.boundingBox.y,
