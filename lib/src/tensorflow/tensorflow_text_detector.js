...
class _TextDetector {
  ...
  _subdivTensor(tensor) {
    return tensor.sub(tf.scalar(255 * DET_MEAN)).div(tf.scalar(255 * DET_STD));
  }
  ...
  async _wordHeatMap(imageData) {
    ...
    return new ImageData(heatmap, detection_size.width, detection_size.height);
  }
    ...
    return new Promise(async (resolve) => {
      ...
      for (let bbox of boundingBoxes) {
        ...
        for (let i = 0, offset; i < height; i++) {
          ...
          crop.set(data.subarray(offset, offset + width * 4), i * width * 4);
        }
